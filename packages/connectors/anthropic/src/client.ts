...
// Pinned API version header required by the Anthropic API.
...
async function anthropicRequest<T>(
  ...
  const res = await ctx.fetch(url, {
    ...
    headers: anthropicHeaders(
      ctx.credentials as AnthropicCredentials,
      body !== undefined,
    ),
  ...
  if (!res.ok) {
    ...
    throw new Error(
      `Anthropic API ${res.status} ${res.statusText} for ${path}${
...
export function anthropicGet<T>(
  ctx: ConnectorContext,
  path: string,
): Promise<T> {
  return anthropicRequest<T>(ctx, "GET", path);
}

export function anthropicPost<T>(
  ctx: ConnectorContext,
  path: string,
  body: unknown,
): Promise<T> {
  return anthropicRequest<T>(ctx, "POST", path, body);
}

/** POST that returns the raw response body stream (for streaming endpoints). */
export async function anthropicFetchStream(
  ctx: ConnectorContext,
  path: string,
  body: unknown,
  ...
  const res = await ctx.fetch(`${config.baseUrl.replace(/\/$/, "")}${path}`, {
    method: "POST",
    headers: anthropicHeaders(ctx.credentials as AnthropicCredentials, true),
  ...
  if (!res.body) throw new Error("Anthropic API: empty response body");
...
export async function anthropicRawRequest(
  ctx: ConnectorContext,
  req: RawApiRequest,
  ...
  if (!res.ok) {
    ...
    throw new Error(
      `Anthropic API ${res.status} ${res.statusText} for ${req.method} ${req.path}${
